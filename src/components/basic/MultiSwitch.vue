<template>
	<div class="relative rounded-md bg-plastic-800 border-2 border-plastic-900">
		<div v-if="activeItemElement" :style="knobStyle"
			class="absolute top-0 w-9 h-9 rounded-md transition-[left] border-y border-t-cyan-600 border-b-cyan-900 bg-gradient-to-b from-cyan-800 to-cyan-600" />
...
function itemClasses(item: MultiSwitchItem) {
	return [
		...(item.active ? ["text-cyan-200"] : ["text-plastic-500", "cursor-pointer"]),
