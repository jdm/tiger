<template>
	<div class="relative rounded-md bg-plastic-800 border-2 border-plastic-900">
		<div v-if="activeItemElement" :style="knobStyle"
			class="absolute top-0 w-9 h-9 rounded-md transition-[left] border-y border-t-sky-600 border-b-sky-900 bg-gradient-to-b from-sky-800 to-sky-600" />
...
function itemClasses(item: MultiSwitchItem) {
	return [
		...(item.active ? ["text-sky-200"] : ["text-plastic-500", "cursor-pointer"]),
