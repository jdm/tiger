<template>
	<div class="relative rounded-md bg-plastic-800 border-2 border-plastic-900">
		<div v-if="activeItemElement" :style="knobStyle"
			class="absolute top-0 w-9 h-9 rounded-md transition-[left] border-y border-t-amber-600 border-b-amber-900 bg-gradient-to-b from-amber-800 to-amber-600" />
...
function itemClasses(item: MultiSwitchItem) {
	return [
		...(item.active ? ["text-amber-200"] : ["text-plastic-500", "cursor-pointer"]),
